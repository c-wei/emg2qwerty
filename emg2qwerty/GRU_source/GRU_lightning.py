...
class GRUCTCModule(TDSConvCTCModule):
    def __init__(
        ...
        if select_channel_indices is not None:
            self.register_buffer(
                "_channel_idx",
                torch.tensor(list(select_channel_indices), dtype=torch.long),
            )
        else:
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        if self._channel_idx is not None:
            # inputs: (T, N, bands=2, C=16, freq) -> select channels on dim 3
            inputs = inputs[:, :, :, self._channel_idx, :]
        return self.model(inputs)
...
class CRNNCTCModule(pl.LightningModule):
    ...
    def __init__(
        ...
        select_channel_indices: Sequence[int] | None = None,
    ) -> None:
        super().__init__()
        self.save_hyperparameters()
