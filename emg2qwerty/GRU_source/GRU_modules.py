...
class GRUEncoder(nn.Module):
    ...
    Args:
        num_features (int): Num features for the input.
        hidden_dim (int): Num hidden units in the GRU.
        num_layers (int): Num GRU layers.
        dropout (float): Dropout probability.
        bidirectional (bool): Use bidirectional or not.
    """

    def __init__(
            # Initial architecture choices based on
            # Yue Chen and Xin Zheng. Second-place solution for the Brain-to-Text Benchmark ’24. GitHub
            # repository, https://github.com/CIBR-Okubo-Lab/speechBCI_2024, 2024.
        self,
        num_features: int,
        hidden_dim: int = 512,
        num_layers: int = 3,
        ...
        self.output_proj = nn.Linear(gru_out_dim, num_features)

    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        x, _ = self.gru(inputs)  # (T, N, gru_out_dim)
        return self.output_proj(x)  # (T, N, num_features)
